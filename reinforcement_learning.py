...
class FlightResourceEnvironment:
    ...
    def __init__(self, flight_data):
        ...
        self.resources = {
            'ground_crew': 20,  # Total available ground crew
            'equipment': 15,     # Total available equipment units
...
class QLearningAgent:
    ...
    def _get_state_key(self, state):
        ...
        state_key = tuple([
            ...
            int(state[2] * 10),  # ground_time_pressure
            int(state[3] * 10),  # transfer_bag_ratio
            int(state[4] * 10),  # ssr_intensity
            int(state[5]),       # is_international
            int(state[6] * 10),  # available_ground_crew
            int(state[7] * 10),  # available_equipment
            int(state[8] * 10)   # available_special_services
        ])
        return state_key
